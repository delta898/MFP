...
- `발행 상태`: Blog Beta의 모든 내부 탭에서 공통으로 현재 글감과 실제 runner 진행 메시지·완료·실패를 표시하고, 실행 중 중복 시작을 차단. 실행 위치의 `상태 보기`를 눌렀을 때만 공통 상태로 이동
...
공통 발행 상태는 평소 숨겨져 있다가 실행이 시작되면 Blog Beta 상단 탭 아래에 나타난다. 현재
