...
16. 카드뉴스 생성·발행
   ...
   - 첫 단계는 이미지 묶음 내보내기와 미리보기로 제한하고, Instagram 등 채널별 자동 발행은 인증·caption·alt text·실패 복구 계약이 검증된 뒤 추가한다.
   - 설정된 Bitly API가 있으면 사용자가 선택적으로 긴 원문 URL을 자동 단축해 SNS 발행 문구에 넣을 수 있게 한다.
