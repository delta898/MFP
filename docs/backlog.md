...
> 현행 기준: 2026-09-04 · 최신 릴리스: `v0.4.1` · 다음 버전: `v0.4.2`
...
19. 실행 중 작업 취소와 안전한 복구
   - 글감·키워드 탐색, 글쓰기, 이미지·스마트 댓글 생성과 블로그 발행처럼 시간이 오래 걸리는 AI·자동화 작업에서 사용자가 현재 실행을 명시적으로 취소할 수 있게 한다.
   - 공통 AI 작업 계약에 취소 신호를 포함하고, 공급자가 요청 취소를 지원하지 않는 경우에도 후속 단계와 결과 반영을 중단하는 일관된 fallback을 제공한다.
   - UI 버튼만 다시 활성화하는 방식이 아니라 작업별 cancel capability와 취소 가능한 단계, 이미 시작된 외부 요청·브라우저 작업의 정리 계약을 정의한다.
   - 취소 전까지 완성된 원고·댓글·이미지 등 유효한 결과는 보존하고 작성 중인 입력과 대기열 항목을 지우지 않는다.
   - `취소 요청 중 → 취소 완료` 상태와 취소할 수 없는 마지막 발행 단계의 차이를 명확히 안내하고, 중복 실행 lock이 반드시 해제되는지 검증한다.

20. Dashboard 계정 연동 현황 안정화
   - Dashboard와 설정 화면이 네이버·WordPress 연결 상태를 서로 다른 기준이나 오래된 cache로 표시하지 않도록 하나의 canonical connection status 계약을 사용한다.
   - 특히 네이버는 저장된 로그인 정보와 실제 세션 유효성을 구분하고, `로그인됨`, `확인 중`, `재로그인 필요`, `확인 실패`를 일관되게 표시한다.
   - 앱 시작 시 전체 화면을 오래 기다리게 하지 않고 마지막 확인 상태를 먼저 보여준 뒤 비동기로 갱신하며, 설정 저장·로그인·로그아웃·접속 확인 결과를 Dashboard에 즉시 반영한다.
   - 일시적인 네트워크 실패를 계정 해제로 단정하지 않고 확인 시각, 재시도와 설정 이동 등 사용자가 취할 다음 행동을 제공한다.

21. 전역 새 글 작성 단축키
   - 메뉴의 `새 글 작성`과 같은 공용 action을 호출하는 macOS·Windows 단축키를 제공한다.
   - `CmdOrCtrl+N`을 우선 후보로 삼되 입력창, modal, 저장하지 않은 설정과 충돌하지 않는지 검증하고 메뉴에도 단축키를 표시한다.
   - 새 블로그의 `빠른 글 작성 → 바로 생성`으로 이동한 뒤 주제 입력에 자연스럽게 초점을 둔다.

22. Electron runtime 업그레이드
   - 현재 Electron 40 계열에서 지원 중인 stable major로 올리고 Node/Chromium 변화와 macOS Apple Silicon 동작을 함께 검증한다.
   - 종료 시 반복되는 native menu `representedObject` 경고가 해소되는지 확인하고, 메뉴 수명주기·window 종료 순서를 별도로 점검한다.
   - macOS·Windows 패키징, 자동 업데이트, 앱 시작·종료와 핵심 UI browser smoke를 통과한 뒤 릴리즈에 포함한다.

23. 글쓰기 완료 목록과 결과 이어보기
   - BlogGenius에서 처리가 끝난 글을 별도 완료 목록에서 다시 확인할 수 있게 하고 대기열·진행 중 작업과 역할을 구분한다.
   - 임시 저장, 예약, 공개 발행과 플랫폼별 성공·실패를 구분하고 제목, 대상 채널, 완료 시각과 최종 결과 링크를 함께 표시한다.
   - 공개 발행은 해당 글로, 임시 저장처럼 글 주소를 확정하기 어려운 결과는 해당 블로그나 관리 화면으로 이동할 수 있게 한다.
   - Google Sheet 상태, local activity event와 플랫폼 결과 중 무엇을 canonical 완료 이력으로 사용할지 정하고 중복 실행이나 앱 재시작 후에도 같은 목록을 재구성한다.
   - 완료 항목에서 결과 열기, 실패 원인 확인, 안전한 재시도와 새 글의 참고 원문으로 사용하기 같은 후속 동선을 제공한다.

24. Installer 역할 분리: 설치 껍데기와 앱 내용
...
16. SNS 원문 URL 선택적 단축
   - 설정된 Bitly API가 있으면 사용자가 긴 원문 URL을 자동 단축해 카드뉴스와 일반 SNS 발행 문구에 넣을 수 있게 한다.
   - 원문 URL을 기본적으로 보존하고, 단축 실패가 카드 생성이나 수동 발행을 막지 않도록 한다.

17. 공통 발행 버튼 상태 계약
   - 블로그, 쇼핑, 카드뉴스와 SNS의 모든 발행 버튼에 `발행 가능 → 발행 중 → 발행 완료 / 실패` 상태를 일관되게 적용한다.
   - 발행 중에는 spinner와 진행 문구를 표시하고 관련 입력·중복 실행을 잠그며, 완료 후 동일한 입력은 다시 발행되지 않도록 한다.
   - 내용, 대상 채널, 이미지, 이미지 순서나 발행 설정이 변경되면 다시 발행할 수 있게 활성화하고, 일부 실패는 성공 채널과 재시도 대상을 구분한다.
   - 각 화면에 흩어진 boolean과 CSS를 복제하지 않고 공통 action-state 계약과 재사용 가능한 UI 패턴으로 정리한다.
