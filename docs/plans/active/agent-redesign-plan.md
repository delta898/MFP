...
## Telegram 대표 기능 실전 점검 체크리스트

> 우선순위 원칙:
> - **핵심 기능**: 글감 등록, 발행 요청
> - **부가 기능**: 트렌드 추천/제안, 성향 요약, provider 활용
>
> Telegram 주제는 핵심 기능이 실전에서 빠르고 안정적으로 동작하는지 먼저 검증한다.

### A. 글감 등록

1. **단순 등록**
   - 예: `애드센스 사용법 주제로 글감 추가해줘`
   - 기대:
     - help/meta 오인식 없이 register intent로 해석
     - 확인 메시지가 등록 목적 기준으로 표시
     - 취소/확인 버튼 정상 동작

2. **주제만 바꾼 반복 등록**
   - 예: `경주 여행 주제로 글감 추가해줘`
   - 기대:
     - 직전 요청과 혼동 없이 새 pending으로 처리
     - 글감 주제만 반영되고 불필요한 publish action이 붙지 않음

3. **플랫폼 없이 등록만 요청**
   - 예: `이 주제는 일단 저장만 해줘`
   - 기대:
     - register only
     - publish action 자동 추가 금지

### B. 등록 후 바로 발행

4. **등록 + 네이버 발행**
   - 예: `경주 맛집 주제로 네이버에 발행해줘`
   - 기대:
     - `register_topic + publish_article`
     - 네이버 타깃 지정
     - 확인 메시지에 의도와 플랫폼 반영

5. **등록 + 워드프레스 발행**
   - 예: `AI 도구 추천 글 워드프레스에 올려줘`
   - 기대:
     - register + publish
     - 워드프레스 타깃 지정

6. **등록 + 전체 발행**
   - 예: `이거 등록하고 바로 발행까지 해줘`
   - 기대:
...
7. **플랫폼별 카테고리 지정**
   - 예: `네이버는 기술, 워드는 Tech로 해서 발행해줘`
   - 기대:
     - `naver_category`, `wordpress_category` 분리 반영
     - confirmation에 category가 일관되게 표시

8. **공통 카테고리 지정**
   - 예: `건강 카테고리로 네이버에 발행해줘`
   - 기대:
     - 공통 category 처리 정상
...
9. **발행 요청 취소**
   - 등록/발행 confirmation에서 취소
   - 기대:
     - pendingRequests 삭제
     - 이후 재요청 시 이전 상태가 섞이지 않음

10. **확인 카드 수정**
   - 플랫폼 선택/대상 변경이 가능한 카드가 있으면 변경 후 다시 confirmation
   - 기대:
     - 업데이트된 confirmation이 반영
     - 이전 state와 충돌 없음

### E. 실패 / 인증 / 회귀

11. **네이버 세션 없음**
   - publish 실행 시 로그인 세션 부족
   - 기대:
     - 사용자에게 원인이 보임
     - 프로세스가 죽지 않음
     - 로그가 남음
...
- 글감 등록/발행 핵심 시나리오가 안정적으로 동작한다.
- help/meta 오인식이 핵심 기능을 방해하지 않는다.
- pending confirmation / 취소 / 대체 흐름이 자연스럽다.
- 느린 응답과 rate limit 상황에서도 봇 프로세스가 죽지 않는다.
- 추천/제안 기능은 부가 가치로 남기되, 핵심 경로를 방해하지 않는다.
