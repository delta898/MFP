...
설정이 아니라 BlogGenius 서비스 자격증명으로 관리한다. 운영 자격증명은
Supabase Edge Function Secrets에만 보관한다.
...
- 운영 호출은 라이선스 인증, Postgres cache, rate limit, backoff가 있는
  `keyword-research` Edge Function을 통한다.
- 별도 Gateway 서버와 중간 접근 토큰은 운영하지 않는다.
