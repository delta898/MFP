...
### Added
- **네이버 트렌드 글감 탐색**: 최신 데이터, 최근 7일 또는 직접 지정한 기간과 최대 5개 카테고리로 네이버 트렌드 키워드를 조회하고, 정렬과 간단한 결과 필터로 필요한 글감을 찾을 수 있습니다.
- **기존 포스팅 흐름 연결**: 선택한 트렌드 키워드를 출처와 반영일 정보와 함께 빠른 포스팅으로 보내거나 Google Spreadsheet `topics`에 `대기` 상태의 글감으로 바로 저장할 수 있습니다.
- **최근 저장 글감 제외**: 최근 15일 안에 `topics`에 저장한 주제와 키워드를 조회 결과에서 선택적으로 제외할 수 있습니다.

### Changed
- **트렌드 결과 탐색 개선**: 결과 필터와 테이블 헤더를 스크롤 중에도 함께 확인할 수 있고, 키워드·변화·작업을 인접 배치해 글감 판단과 후속 작업 동선을 단순화했습니다.
- **글감 저장 반응성 개선**: 단건 저장에서 불필요한 전체 시트 점검과 고정 대기를 제거하고, 저장 진행·성공·실패 상태를 해당 결과 행에 표시합니다.
