...
## [0.4.3-dev6] - 2026-09-06

### Fixed
- **Windows 자동 업데이트 적용 복구**: 업데이트 후 앱만 다시 열리고 이전 버전이 유지되던 문제를 수정하고, 적용 실패 시 원인을 확인할 수 있도록 단계별 기록을 보강했습니다.
...
- **카드뉴스 관리와 이어서 작업하기**: Google Sheet의 `cardnews` 관리 대장으로 아직 만들지 않은 글, 만든 카드뉴스, 발행 결과와 게시물 링크를 구분해 볼 수 있습니다. RSS에서 같은 글이 다시 들어와도 기존 상태를 유지합니다.
- **다양한 카드뉴스 소스**: 연결된 네이버·WordPress 블로그뿐 아니라 로그인 없이 접근 가능한 HTTPS RSS를 추가해 카드뉴스 소재로 사용할 수 있습니다.
- **카드뉴스 ZIP 가져오기와 내려받기**: 완성한 카드 이미지를 제목이 포함된 ZIP으로 내려받고, 외부에서 만든 ZIP 카드뉴스도 가져와 이어서 관리하거나 발행할 수 있습니다.
- **SNS 다중 이미지 발행**: 로컬 이미지와 카드뉴스 이미지를 여러 장 선택해 미리 보고 순서를 바꾼 뒤 Buffer 채널에 발행할 수 있습니다. WordPress 설정 없이도 Google Drive 연동으로 필요한 공개 이미지 링크를 준비합니다.

### Changed
- **카드뉴스 기본 동선 단순화**: 내부 작업 용어를 줄이고, 글을 선택하면 바로 내용을 확인한 뒤 카드 구성과 이미지 생성으로 이어지도록 정리했습니다.
- **카드뉴스 이미지 작업 개선**: 이미지 생성 중에는 중복 실행을 막고 진행 상태를 보여주며, 이미 만든 이미지는 필요한 카드만 다시 만들거나 내 이미지로 바꿀 수 있습니다.
- **SNS 발행 안내 개선**: 발행 문구와 해시태그를 함께 준비하고, 발행 중·발행 완료 상태와 채널별 게시물 링크를 더 명확하게 보여줍니다.
- **데스크톱 실행 환경 업데이트**: 지원이 종료된 Electron 40을 Electron 44로 올렸습니다. v0.4.3부터 macOS는 13(Ventura) 이상을 지원하며 Windows는 기존과 같이 64비트를 지원합니다.
