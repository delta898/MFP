...
### Added
- **나만의 글쓰기 프로필**: 설정에서 기본 프로필과 내 프로필을 선택할 수 있습니다. 내 프로필에는 검색·발견 중심 전략, 표현 방식, 높임 방식, 본문 길이와 전개, 도입·마무리, 이미지 영역 수와 추가 작성 원칙을 저장해 블로그와 쇼핑 글에 함께 적용할 수 있습니다.
- **참고 글로 자동 설정**: 잘 쓰인 글을 붙여넣거나 네이버 블로그 URL 하나를 입력하면 AI가 문체, 길이, 구성과 전개 방식을 분석해 내 프로필을 자동으로 채웁니다. 분석된 값은 이후 원하는 대로 직접 조정할 수 있습니다.
- **글쓰기 설정 미리보기**: 현재 화면의 설정으로 일반 블로그 또는 쇼핑 글 샘플을 생성해 실제 글에 어떻게 적용될지 저장 전에 확인할 수 있습니다. 쇼핑 미리보기는 고정된 예시 상품을 사용합니다.
- **글별 이미지 처리 선택**: 글을 작성할 때 `이미지 생성`, `이미지 프롬프트만 포함`, `이미지 사용 안 함` 중 하나를 선택할 수 있습니다. 빠른 포스팅뿐 아니라 글감·표·수정, 일괄 및 자동 발행에서도 같은 기준을 사용합니다.
- **뜻밖의 발견**: 대시보드에서 네이버 트렌드와 뉴스, 최근 콘텐츠 활동을 바탕으로 지금 써볼 만한 소재를 제안합니다. 제안 근거를 확인하고 바로 글감에 적용하거나 관심 없음으로 제외할 수 있습니다.
- **내 기록을 발전시킨 후속 소재**: 저장·선택·작성·발행했던 주제를 그대로 반복하지 않고, 후속편·비교·체크리스트·다른 관점처럼 다음 글로 발전시킨 제안을 제공합니다.
- **발견 자동 갱신**: 앱을 시작할 때 새로운 발견을 준비하며, 대시보드의 `새로운 발견` 버튼으로 언제든 다시 찾을 수 있습니다.

### Changed
- **글쓰기 설정 화면 단순화**: 기본 프로필에서는 사용자가 바꿀 수 있는 글 작성 전략과 어투만 보여주고, 세부 구성은 내 프로필에서만 설정하도록 정리했습니다. 중복 버튼·상태 문구·설명을 줄이고 입력창과 선택 항목의 모양을 다른 화면과 맞췄습니다.
- **참고 글 분석 정보 강화**: AI 분석 결과에 전개 방식을 함께 표시하고, 분석 완료 시각을 한국 시간으로 보여주며 실제 사용한 글쓰기 모델도 확인할 수 있습니다.
- **네이버 블로그 URL 호환성 확대**: PC·모바일·PostView 형태의 네이버 블로그 주소를 모두 인식해 분석 가능한 모바일 주소로 정리합니다.
- **미리보기 결과 표현 개선**: 문단과 줄바꿈, 이미지 프롬프트 영역을 실제 글과 가깝게 보여줍니다. 샘플이 목표 글자 수를 조금 벗어나도 결과를 제공하고, 지나치게 긴 경우에는 읽기 좋은 범위로 다듬어 표시합니다.
- **발견 카드 품질 개선**: 사용자 활동에 근거한 카드에는 가장 최근 근거 하나만 간결하게 표시하며, 제외한 카드 자리에는 새로운 제안을 채웁니다.
- **한국어 뉴스 우선 제공**: 별도의 AI 비용을 사용하지 않고 뉴스 제목의 한글 여부를 확인해 한국어 기사만 제안합니다. 해외 이슈도 한국어 제목이라면 발견 대상으로 포함됩니다.
- **AI 제목 추천 이동 개선**: 선택 키워드로 제목을 생성하면 추천 결과 영역으로 자동 이동해 새 결과를 바로 확인할 수 있습니다.

### Fixed
- **참고 글 분석 안정성 개선**: 긴 네이버 URL, 긴 분석 응답과 일부 비정형 AI 응답을 더 안정적으로 처리합니다. 분석에 실패해도 이전 분석 결과와 현재 프로필 설정은 유지됩니다.
- **AI 작업 진행 상태 개선**: 참고 글 분석과 미리보기, 새로운 발견을 실행하는 동안 버튼과 진행 문구에서 작업 중임을 명확히 확인할 수 있으며 중복 실행을 막습니다.
- **미리보기 이전 결과 초기화**: 새 미리보기를 시작하면 기존 샘플을 먼저 비워 오래된 결과를 새 결과로 오해하지 않도록 했습니다.
- **참고 글 화면 표시 수정**: 긴 주소와 상태 메시지가 화면을 벗어나지 않으며, 참고 글을 지울 때 무엇이 삭제되고 어떤 설정이 유지되는지 분명하게 안내합니다. 사용 중인 입력 탭도 임의로 바뀌지 않습니다.
- **쇼핑 미리보기 입력 혼동 수정**: 쇼핑 선택 시 사용하지 않는 주제 입력란을 비활성화하고 고정 예시 상품을 사용한다는 안내를 표시합니다. 미리보기 버튼의 위치와 크기는 블로그와 동일하게 유지됩니다.
- **빠른 포스팅 여백 수정**: 글 작성 전략을 바꿀 때 화면 아래에 불필요한 빈 공간이 생기던 문제를 해결했습니다.
- **발견 진행 표시 통일**: `새로운 발견` 실행 중 별도 문구 대신 버튼 안에 회전 표시와 `발견 중...` 상태를 보여줍니다.

## [0.2.0] - 2026-08-22

### Added
- **개인화 글감 추천**: 빠른 포스팅에서 최근 글쓰기와 발행 활동, 관심 주제, 네이버 트렌드를 바탕으로 근거와 추천 이유가 있는 글감 3개를 제안합니다. 추천을 바로 빠른 포스팅에 적용하거나 글감으로 저장하고, 다른 추천을 요청하거나 불필요한 제안을 제외할 수 있습니다.
- **추천 결과 학습**: 추천 선택, 글감 저장, 초안 작성, 실제 발행과 명시적인 제외 의견을 소유자별 이벤트로 기록해 이후 추천 품질을 개선합니다. 단순 노출이나 예약만으로는 관심사 또는 발행 성공으로 간주하지 않습니다.
- **통합 글감·키워드 탐색**: 빠른 포스팅의 하나의 탐색 화면에서 `글감 추천`과 `키워드 탐색`을 전환할 수 있습니다. 시작 키워드를 직접 입력하거나 비워 두고 최근 트렌드, 관심 주제와 최근 글쓰기 신호에서 자동으로 후보를 찾을 수 있습니다.
- **네이버 키워드 지표 분석**: 입력 키워드와 연관 키워드의 월간 검색량, 추정 주간 검색 수, 최근 7일 신규 문서 수, 경쟁도와 기회지수를 비교하고 최대 3개를 선택할 수 있습니다.
- **AI 제목 추천**: 선택한 글감과 키워드를 바탕으로 역할과 장단점이 구분된 제목 후보를 생성하고, 선택한 제목과 키워드를 빠른 포스팅에 한 번에 적용할 수 있습니다.
...
- **지능형 메모리 기반 확장**: 로컬 소유자 식별, 블로그·쇼핑·SNS·Telegram 활동 lifecycle, 피드백 출처, 주제 의미와 관심 프로필을 이벤트 우선 구조로 연결해 추천과 Agent Runtime이 동일한 기억을 사용합니다.
- **운영형 추천 콘텐츠 영역**: Supabase에서 플랜별 사이드바, 대시보드와 내 정보의 추천 자료·개발자 소식·후원 항목을 관리할 수 있습니다. 앱 재배포 없이 문구, 링크, 순서, 노출 대상과 게시 상태를 변경할 수 있습니다.
- **오늘의 추천 자료**: 대시보드에 사용자의 플랜에 맞는 일일 추천 자료를 표시하고, 앱으로 돌아오면 최신 운영 콘텐츠를 자동으로 갱신합니다.
- **집중 타이머와 시계 꾸미기**: 시계에서 25분 집중·5분 휴식 타이머를 실행하고 완료 효과를 확인할 수 있으며, 시계 표시 방식과 계절별 테마를 선택할 수 있습니다.
- **빠른 포스팅 완료 효과**: 포스팅 작업이 성공하면 완료 상태를 더 명확하게 확인할 수 있는 축하 효과를 표시합니다.
- **Gemini 3.7 Flash 지원**: Google AI 글쓰기 모델 목록에 Gemini 3.7 Flash를 추가하고 기존 Gemini 호출 경로와 연결 확인, reasoning 설정을 그대로 지원합니다.

### Changed
- **추천 생성 품질 개선**: 메모리와 지식 공급자에서 얻은 근거로 후보를 생성하고 유사한 제안을 줄이는 다양성 정책과 설명 가능한 순위를 적용했습니다. 추천은 사용자가 요청할 때만 생성하며 일반 화면 전환에서는 캐시를 재사용합니다.
- **키워드 분석 보안 구조 개선**: 네이버 Search Ads와 Blog Search 자격증명을 데스크톱 앱에서 제거하고, 라이선스 검증·요청 제한·TTL 캐시가 적용된 Supabase Edge Function을 통해 원본 지표만 조회하도록 변경했습니다.
- **기회지수 정확성 개선**: 최근 문서 조회가 상한에 도달한 키워드는 정확한 점수처럼 표시하지 않고 계산 가능한 상한을 `N 이하`로 표시하며, 정확값 정렬에서는 제외합니다.
- **빠른 포스팅 입력 흐름 정리**: 글감, 키워드와 제목의 역할을 분리하고 탐색 결과를 적용할 때 작성 중인 내용을 덮어쓸지 먼저 확인하도록 정리했습니다.
- **운영 콘텐츠 안정성 개선**: 동적 콘텐츠 조회가 실패해도 핵심 메뉴와 마지막 정상 콘텐츠를 유지하며, 창 포커스 복귀 시 갱신하고 긴급 일시중지와 복원 정책을 지원합니다.
- **사이드바 사용성 개선**: 동적 콘텐츠가 추가되어도 스크롤과 핵심 메뉴 접근이 안정적으로 유지되며 개발자 지원 항목의 식별성을 높였습니다.

### Fixed
- **글감 추천 자동 실행 방지**: 빠른 포스팅 진입만으로 AI 추천과 사용량이 실행되지 않도록 하고 사용자의 명시적인 요청에서만 시작합니다.
- **글감·키워드 입력 혼동 수정**: 주제와 키워드 입력이 서로 덮어쓰거나 잘못 해석되는 경로를 분리하고 기존 빠른 포스팅 입력과의 충돌을 확인합니다.
- **키워드 지표 실패 안내 개선**: 네이버 상위 서비스 장애, 요청 제한과 일부 지표 누락을 구분해 안내하고, 분석이 실패해도 입력한 글감과 키워드로 제목 추천을 계속할 수 있게 했습니다.
- **추천 근거와 활동 출처 정합성 개선**: Telegram, 블로그, 쇼핑과 SNS 이벤트의 소유자·대상·결과 출처를 정규화해 다른 사용자의 신호나 단순 예약 상태가 추천 학습에 섞이지 않도록 했습니다.

## [0.1.18] - 2026-08-12

### Added
- **네이버 트렌드에서 글감 찾기**: 조회 기간과 관심 카테고리를 선택해 최신 트렌드 키워드를 찾아볼 수 있습니다. 키워드 검색과 상승·하락·신규 등의 필터, 항목별 정렬로 원하는 글감을 빠르게 좁힐 수 있습니다.
- **찾은 글감으로 바로 시작하기**: 마음에 드는 키워드는 빠른 포스팅으로 바로 작성하거나, 나중에 사용할 글감으로 저장할 수 있습니다.
- **이미 저장한 글감 제외하기**: 최근 15일 안에 저장한 글감을 결과에서 숨겨 새로운 주제에 집중할 수 있습니다.

### Changed
- **트렌드 결과를 더 편하게 탐색**: 긴 결과를 스크롤해도 필터와 항목 제목을 계속 확인할 수 있으며, 키워드의 변화와 다음 작업을 한눈에 비교할 수 있도록 화면 구성을 정리했습니다.
- **글감 저장 속도와 안내 개선**: 글감 저장 시간을 줄이고, 저장 중·완료·실패 상태를 선택한 글감에서 바로 확인할 수 있습니다.

## [0.1.17] - 2026-08-11

### Added
- **쇼핑커넥트 참고·지시사항**: 빠른 포스팅과 일괄·자동 포스팅에서 상품별 참고·지시사항을 선택적으로 입력하고, 사용자 지시를 쇼핑 콘텐츠 생성에 우선 반영할 수 있습니다.
- **자동 포스팅 저장 방식 선택**: 블로그 자동 포스팅 설정에서 즉시 발행 또는 임시 저장을 선택하고 예약 실행과 수동 실행에 동일하게 적용할 수 있습니다.
- **검색·발견 중심 글쓰기 전략**: 블로그 글을 검색 의도와 정보 구조에 맞추는 검색 중심 또는 피드에서 독자의 관심과 읽기 흐름을 고려하는 발견 중심으로 생성할 수 있습니다. 전역 기본값과 빠른 글 작성·글감별 선택을 지원하며, 공통 JSON·본문·이미지 계약 위에 전략별 프롬프트를 분리했습니다.
- **스마트 댓글 후보 수집 및 초안 개선**: 저장된 네이버 로그인 세션으로 이웃새글의 제목과 본문 일부를 수집하고, 글의 맥락에 맞는 한국어 댓글 초안 3개를 구조화된 응답으로 생성합니다.

### Changed
- **쇼핑 콘텐츠 품질 개선**: 공통 문체와 검색·발견 중심 전략을 쇼핑 글에도 적용하고, 근거에 따라 글의 구성과 전개를 다양화해 반복적이고 무미건조한 표현을 줄였습니다.
- **쇼핑 제목 정확도 강화**: 입력하거나 수집한 실제 상품명을 제목에 반영하고, 키워드 중복과 일반적인 서비스명이 상품명으로 사용되는 문제를 줄였습니다.
- **블로그 문체 예시 제공**: 표현 방식과 높임 방식의 조합에 따라 예상 문체 설명과 실제 문장 예시를 함께 표시해 구어체·문어체와 존댓말·평어의 차이를 바로 비교할 수 있습니다.
- **Buffer 안내 동선 개선**: SNS 즉시 발행 화면의 안내 링크를 필요한 위치에 간결하게 배치하고, 설정 화면과 발행 채널 안내에서 Buffer 소개 및 가입 경로를 확인할 수 있게 정리했습니다.

### Fixed
- **스마트 댓글 요청 한도 대응**: 여러 후보의 댓글을 배치 요청으로 생성하고 AI 공급자의 재시도 대기 시간을 반영합니다. 최종 요청 한도 초과 시 남은 배치를 중단하며 화면에 배치 진행률과 대기·중단 상태를 표시합니다.
...
- **Markdown 원고 붙여넣기 포스팅**: 빠른 포스팅을 `바로 생성`, `원고 폴더`, `원고 붙여넣기`로 나누고, 제목과 본문이 포함된 Markdown을 UI에 직접 붙여넣어 미리보기·검증 후 기존 네이버·워드프레스 발행 흐름으로 실행할 수 있습니다. 입력 중인 원고는 브라우저에 임시 보존되며, 발행 전 RSS 기반 관련 글 3개를 연관도 우선·랜덤 보충 방식으로 원고 하단에 자동 추가합니다.

### Fixed
- **GitHub Actions 릴리스 패키징 안정화**: Node 24에서 산출물을 만들지 못하던 deprecated `electron-packager`를 유지보수 중인 `@electron/packager`로 교체하고, 잠금 파일 기반 설치와 플랫폼별 아이콘 지정, 패키징 결과 경로 검증을 추가했습니다.
- **사용자 지정 업데이트 서버 즉시 반영**: 앱 실행 중 업데이트 서버 URL 또는 채널을 변경해도 Updater가 시작 시점 설정과 이전 조회 캐시를 계속 사용하던 문제를 수정했습니다.
- **원고 이미지 누락 시 임시 저장 허용**: 빠른 포스팅의 원고 선택에서 이미지 블록에 대응하는 파일이 없어도 실행을 차단하지 않고 경고만 표시하며, 기존 네이버·워드프레스 발행 안전장치에 따라 원고 본문을 임시 저장할 수 있도록 수정했습니다.

### Changed
- **Google AI 모델 카탈로그 최신화**: Gemini 글쓰기 모델을 `Gemini 3.5 Flash`, `Gemini 3.1 Flash-Lite`, `Gemini 3.1 Pro Preview` 중심으로 정리하고, 이미지 모델을 안정화된 `Nano Banana 2`, `Nano Banana Pro` 코드로 전환했습니다. 종료된 Gemini 2.5 글쓰기 모델과 preview 모델은 신규 선택 목록에서 제거하되 기존 사용자 선택은 명시적으로 변경하기 전까지 보존됩니다. 아직 충분한 지원 기간이 남은 `Nano Banana (Gemini 2.5)`는 이미지 모델 선택지로 유지합니다.
...
### Added
- **Stable latest 다운로드 별칭**: stable 릴리즈에서 각 플랫폼 ZIP을 `BlogGenius-<platform>-latest.zip` 별칭으로도 함께 제공해, 사용자 안내 링크를 버전 고정 파일명 대신 최신 stable 링크로 더 쉽게 유지할 수 있도록 했습니다.

### Changed
- **쇼핑커넥트 글 품질 보강**: 쇼핑 상품 페이지에서 공식 상품정보와 사용자 반응 정보를 더 명확히 분리해 제목, 가격, 할인율, 배송비가 상품 페이지의 실제 정보와 더 일관되게 연결되도록 다듬었습니다. 쇼핑 제목 프롬프트도 SEO 키워드 우선, 제목형 마감, 짧고 명확한 소제목 기준으로 다시 정리했습니다.
- **쇼핑 스크랩/발행 동작 분리**: 쇼핑커넥트의 상품 스크랩은 항상 headless로 수행하고, 네이버 블로그 발행 단계는 기존 설정된 headed/headless 정책을 그대로 따르도록 실행 경계를 정리했습니다.
- **사용자 로그 가독성 정리**: 이미지 뒤 Enter, 정렬 복구, 카드 정렬 같은 내부 구현 위주의 메시지는 debug 성격으로 낮추고, 실제 사용자 입장에서 의미 있는 진행 로그가 더 잘 보이도록 조정했습니다.

### Fixed
- **쇼핑 가격/제목 오인식 수정**: 스토어 문구, 리뷰 이벤트 금액, 개인화 할인 금액이 공식 상품명/판매가로 잘못 들어가던 문제를 수정해, `네이버 스마트스토어`류 제목 오염과 `1,000원`, `29,800원` 같은 잘못된 가격 채택을 크게 줄였습니다.
- **인용구 뒤 정렬 회귀 완화**: 네이버 블로그 writing에서 인용구 뒤 다음 블록이 가운데 정렬을 이어받던 문제를 줄이기 위해, 인용구 탈출 후 빈 문단을 만든 다음 좌정렬을 다시 적용하는 흐름으로 보정했습니다.
- **쇼핑 quick publish 브라우저 유지 정책 정합성 보강**: 빠른 쇼핑 포스팅도 일반 블로그 포스팅처럼 마지막 headed 작업에서는 브라우저를 열어두고 사용자가 직접 검토·수정 후 닫을 수 있도록 맞췄습니다.

## [0.1.8-dev11] - 2026-04-04
...
### Added
- **연관 글 선택 휴리스틱**: 블로그/쇼핑 본문 하단의 `함께 보면 좋은 글` 영역이 RSS/목록 후보 중 현재 글의 제목·키워드·본문과 더 연관성이 높은 글을 먼저 고르고, 부족한 수량만 랜덤으로 보충하도록 개선했습니다.

### Fixed
- **빠른 포스팅 발행 옵션/완료 로그 정합성 수정**: 네이버 임시저장/예약 등록이 실제로는 정상 동작했는데도 결과 로그에 `발행 완료`, `즉시 발행`처럼 잘못 표시되던 문제를 수정해, 포스팅 옵션·실행 모드·완료 상태가 실제 동작과 일치하도록 맞췄습니다.
- **빠른 포스팅 로그 버퍼 확장**: 하단 진행 로그가 너무 빨리 잘리고, 스크롤 중에도 새 로그가 들어올 때 강제로 맨 아래로 내려가던 문제를 개선해 더 긴 로그를 유지하면서 과거 로그를 직접 읽을 수 있게 했습니다.
...
### Added
- **주요 이슈 토스트 알림**: 네이버 로그인 만료처럼 즉시 인지가 필요한 상태를 웹/앱 공통 toast로 노출하는 재사용 가능한 UI 이슈 알림 체계를 추가했습니다. 대시보드 세션 상태 polling에서도 같은 규칙을 사용해, 단순 로그 출력만으로 놓치기 쉬운 문제를 더 잘 드러내도록 보완했습니다.

### Changed
- **자동화 UI 제목 스타일 통일**: `트렌드 자동 수집 사용`, `RSS 수집 사용`, `자동 포스팅 사용` 섹션의 체크박스 제목 폰트 크기와 굵기를 공통 스타일로 정리해 블로그/쇼핑 자동화 화면의 위계를 더 일관되게 맞췄습니다.
- **런타임 구조 단순화**: 레거시 CLI 런타임을 제거하고, UI 중심 런처 구조와 `ui-runtime` 경계를 기준으로 런타임 책임을 재정리했습니다. 이에 따라 인증, 자동 사이클, publish/trend/content 액션, HTTP 서버와 API 조립 경계가 더 명확해졌습니다.

## [0.1.8-dev5] - 2026-03-25

### Changed
- **설정 저장 UX 정리**: 블로그/쇼핑 자동 포스팅 관련 저장 액션을 필요한 탭에만 노출하도록 정리하고, 저장 전 변경사항이 탭 이동만으로 사라지지 않도록 draft 상태를 유지하도록 보완했습니다.
- **블로그 이미지 프롬프트 문맥 보정**: 이미지 프롬프트가 본문 맥락을 우선 따르도록 다듬고, 한국어 일상/국내 정보성 글에서는 한국 기반의 현대 장면을 우선 해석하도록 정리했습니다.

### Fixed
- **네이버 로그인 세션 보존 강화**: 인증 상태를 사용하는 자동화 흐름이 최신 세션 쿠키를 다시 저장하도록 보완해, `naver_auth.json` 스냅샷이 오래되어 세션이 불필요하게 풀리는 문제를 줄였습니다.
- **자동 포스팅 스케줄 계산 안정화**: 블로그/쇼핑 자동 발행이 허용 시간대 밖에서 시작될 때 다음 허용 시각을 우선 예약하도록 조정하고, 허용 시간대 변경 시에도 예약 시각이 더 자연스럽게 다시 계산되도록 개선했습니다.
- **쇼핑 자동 포스팅 설정 정합성 수정**: 쇼핑 자동 포스팅 화면과 런타임이 같은 설정 키를 사용하도록 정리해, 발행 주기/건수/허용 시간대가 UI와 실제 실행에서 어긋나던 문제를 바로잡았습니다.
...
### Added
- **글쓰기 AI 모델 선택 구조**: `설정 > AI`에서 텍스트/이미지 글쓰기 모델을 각각 선택할 수 있도록 지원합니다. Gemini/Claude 프리셋과 직접 입력(OpenAI-compatible) 구성을 분리하고, 선택한 모델명·Base URL·API Key를 실제 블로그/쇼핑 글 생성과 이미지 생성에 연결했습니다.
- **브랜드형 Footer 추가**: 앱 하단에 `BlogGenius`, 버전 정보, `@도전인생 All rights reserved`, 카카오 단톡방/네이버 블로그/Threads 외부 링크를 담은 footer를 추가해 화면 완성도를 높였습니다.
- **시계 스타일 확장**: 상단 시계에 7가지 스타일을 제공하고, 앱 시작 시 랜덤 스타일을 선택하도록 확장했습니다. 분리형 숫자 스타일에는 플립 애니메이션을, 스타일 변경 시에는 짧은 pulse 피드백을 추가했습니다.

### Changed
- **AI 설정 구조 정리**: `ai_settings`는 이제 `TEXT_MODEL`, `IMAGE_MODEL`, `CHAT_MODEL` 구조를 사용하고, 상위 기본 설정 키는 `essential` 대신 `general`을 기준으로 정리했습니다.
- **AI 설정 UX 단순화**: 글쓰기 모델 UI를 `AI 선택 / 모델 선택 / Base URL / API Key` 흐름으로 단순화하고, 텔레그램 채팅 모델도 `글쓰기 모델 차용 / CHAT_MODEL 사용` 개념으로 맞췄습니다.
- **쇼핑 글 생성도 글쓰기 모델 공유**: 쇼핑커넥트 글 생성도 블로그와 동일한 글쓰기 모델 설정을 사용하도록 통일했습니다.
- **로그 가독성 개선**: 생성 시작 요약 블록과 모델 호출 로그에 실제 사용 모델 정보를 표시하고, 활동 이력 시간 표현을 `YYYY-MM-DD HH:mm` 형식으로 정리했습니다.

### Fixed
- **패키징 앱 새로고침 복구**: 패키징 앱에서 `Cmd+R` 이후 `Not Found`로 고착되던 문제를 수정하고, 깊은 경로 진입 시에도 UI shell로 복구되도록 보완했습니다.
- **업데이트 배너/강제 업데이트 회귀 수정**: 강제 업데이트 결과가 일반 최신 버전 확인에 섞이거나, 같은 버전인데도 새 버전 배너가 남는 문제를 바로잡았습니다. 업데이트 발견 후 상단 배너로 자연스럽게 이동하는 흐름도 함께 정리했습니다.
- **미리보기 링크 렌더링 개선**: quick/manuscript preview에서 markdown 링크와 bare URL을 실제 클릭 가능한 링크로 렌더하도록 개선했습니다.
- **비밀값 부분 마스킹 통일**: AI/MCP/워드프레스/알림 설정의 토큰/비밀번호/UI secret 표시를 공통 부분 마스킹 방식으로 통일했습니다.
