...
### Added
- **RSS 개별 주기 자동 수집**: 각 피드마다 설정된 주기(분)에 맞춰 독립적으로 자동 수집을 수행하는 스케줄러 구현.
- **RSS UI 고도화**: 카드 형태를 테이블(Table) 형식으로 개편하고, 글로벌 활성화 토글 및 전체 선택/삭제 기능 추가.
- **트렌드 설정 UI 최적화**: 필터 조건, 중복 방지, 스케줄 설정을 논리적으로 그룹화하여 우측 배치.
- **웹 UI 브랜딩**: 페이지 타이틀을 `BlogGenius WebUI`로 변경하고 전용 파비콘(Favicon) 적용.

### Changed
- **RSS 입력 가독성 개선**: URL 및 키워드 입력 필드에 좌측 여백(Padding)을 추가하여 시인성 향상.
- **RSS 필터 로직 정교화**: 피드별 '포함' 및 '제외' 키워드 동시 적용 및 대소문자 구분 없는 필터링 강화.

### Fixed
- **RSS 설정 유지 오류**: 글로벌 활성화 상태(`COLLECT_RSS_ENABLED`)가 저장 또는 로드되지 않던 현상 수정.
- **로그 메시지 포맷 수정**: 하단 로그 영역에서 줄바꿈 문자(`\n`)가 리터럴로 노출되던 가독성 문제 해결.
