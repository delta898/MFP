...
### Added
- **전역 새 글 작성 단축키**: 어느 화면에서든 macOS는 `Cmd+N`, Windows는 `Ctrl+N`으로 블로그의 빠른 글 작성을 열 수 있습니다. 작성 중인 설정이나 열린 대화상자는 안전하게 보호합니다.
- **최신 AI 모델 선택**: GPT-6 Astra·Sol·Luna, Gemini 3.8 Flash와 최신 Claude 모델을 글쓰기에 사용할 수 있습니다. GPT Image 2.5와 Nano Banana 2 Lite 등 최신 이미지 모델도 OpenAI·Google·KIE.ai 선택지에 추가했습니다.

### Changed
- **작업별 AI 품질 자동 조절**: 본문 작성과 분석은 완성도를, 제목·댓글·해시태그 같은 짧은 작업은 속도를 우선하도록 지원 모델의 reasoning 수준을 자동 조절합니다. 적용 방식은 AI 설정에서 확인할 수 있습니다.
- **AI 모델 목록 정리**: 새 선택에는 현재 지원 모델을 우선 표시하고 이전 세대 모델은 목록에서 정리했습니다. 이미 저장된 기존 선택은 바꾸기 전까지 계속 해석할 수 있습니다.
- **앱 창 제목 정리**: macOS에서는 화면 안의 제품명과 중복되던 창 제목을 숨기고, Windows와 브라우저에서는 `BlogGenius`로 간결하게 표시합니다.

### Fixed
- **AI 설정 즉시 반영**: Gemini 모델을 바꾼 뒤 앱을 재시작하지 않아도 실제 생성이 새 모델 주소를 사용합니다. URL 오류에는 API Key를 제외한 요청 주소와 사용 모델을 남겨 원인을 확인하기 쉬워졌습니다.
- **진단 로그 위치 통일**: 날짜별 로그와 Dashboard 활동 기록이 앱의 실제 진단 로그 폴더를 함께 사용하며, AI 역할 저장 기록에 공급자와 모델 이름을 표시합니다.
- **시작 화면 스타일 유지**: 저장한 화면 스타일이 앱 시작 직후부터 적용되어 기본 스타일이 잠깐 나타났다 바뀌는 현상을 줄였습니다.
- **설정 변경 폐기 후 이동**: 저장하지 않은 설정을 버리고 다른 화면으로 이동할 때 멈춘 것처럼 기다리거나 설정에 남던 문제를 고쳤습니다. 폐기한 값은 마지막 저장값으로 즉시 돌아갑니다.

## [0.5.1] - 2026-09-21

### Added
- **미니멀 화면 스타일**: 색과 장식을 걷어낸 가장 단순한 화면을 외모 설정에서 선택할 수 있습니다.
- **저장 후 이동·종료**: 설정 변경사항이 있을 때 떠나기 전에 저장할 수 있습니다. 앱내 이동 확인과 종료 확인이 같은 방식으로 묻습니다.
- **연결 실패 사유 표시**: WordPress 연결이 실패하면 `확인 필요` 뭉뚱그림 대신 실패와 사유를 보여줍니다.

### Changed
- **함께 보면 좋은 글 복구**: 블로그 Beta 미리보기에 보이던 연관글이 발행물에서 빠지던 문제를 고쳐 미리보기와 발행물이 같아졌습니다.
- **미설정 블로그 바로가기**: 대시보드·계정·발행 화면에서 미설정 블로그를 누르면 설정의 블로그 발행 채널로 바로 이동하고 해당 입력 위치까지 스크롤됩니다.
- **연결 상태 표시 개선**: Dashboard Beta 연결 상태가 브랜드 아이콘으로 표시됩니다. 연결되면 브랜드 색, 미설정이면 회색으로 한눈에 구분됩니다.
- **연결 확인 유지**: WordPress·스프레드시트·AI 모델은 한 번 확인하면 앱을 재시작해도 상태가 유지됩니다. AI는 공급자·모델별로 기억하고, 키나 모델이 바뀌면 다시 확인합니다.

### Fixed
- **설정 화면 이동 정리**: 블로그 연결 항목을 눌렀을 때 엉뚱한 설정 탭이나 이전 설정 화면으로 가던 문제를 고쳤습니다.
- **종료 함정 해소**: 설정을 바꾸기만 하고 저장하지 않은 채 종료하면 조용히 막히던 문제를 고쳤습니다. 종료 시 저장 여부를 묻고, 취소해도 앱이 완전하게 살아있습니다.

## [0.5.0] - 2026-09-20

### Added
- **원고와 이미지 통합 미리보기**: 바로 생성, 원고 폴더, 원고 붙여넣기에서 같은 흐름으로 완성 원고를 확인할 수 있습니다. 이미지마다 AI 재생성, 내 이미지 선택, 교체, 사용 안 함, 원래 이미지 복원과 프롬프트 확인·복사를 지원합니다.
- **다섯 가지 화면 스타일**: 따뜻한 에디토리얼, 고요한 세이지 스튜디오, 가을밤 서재, 한지 위의 단청, 레트로 터미널 중 원하는 화면 분위기를 설정에서 선택할 수 있습니다.
- **통합 설정 화면**: 블로그 연결, 작업별 AI 모델, 글쓰기 기본값, 알림·외부 서비스, 앱 실행과 업데이트 설정을 목적별로 찾고 관리할 수 있습니다.

### Fixed
- **Windows 첫 실행 복구 강화**: 선택 기능이나 네트워크가 실패해도 첫 화면을 우선 표시하고, 일반 실행이 불안정하면 안전 모드로 복구할 수 있습니다. 앱 시작 전 단계부터 진단 기록을 남기며 문제 보고에 필요한 로그 묶음을 안내합니다.
- **텔레그램 연결 안정화**: 일부 네트워크의 IPv4/IPv6 연결 차이를 자동으로 보완하고, 토큰·Chat ID·메시지 전송 단계를 나눠 실제 실패 원인을 안내합니다. 수신 오류는 간격을 두고 재시도하며 내부 연결 오류를 대화 메시지에 그대로 노출하지 않습니다.

### Changed
- **빠른 글 작성 흐름 통일**: 세 가지 작성 방식 모두 `내용 준비 → 원고 미리보기 → 발행 설정 → 포스팅` 순서로 진행합니다. 하나의 원고는 하나의 플랫폼에 발행하며, 글감 보관·발행 대기열과 완성 원고 포스팅을 명확히 구분했습니다.
- **빈 이미지 자동 완성**: 이미지 프롬프트가 있지만 이미지가 없는 영역은 포스팅 직전에 AI로 자동 생성합니다. 생성에 실패한 영역이 남은 경우에만 공개·예약 글을 안전하게 임시 저장하며, 사용자가 의도적으로 이미지를 사용하지 않은 영역은 최종 발행을 막지 않습니다.
- **주요 작업 화면 정돈**: Dashboard, 블로그, 카드뉴스, 쇼핑커넥트, SNS, 계정, 설정과 도움말의 제목·단계·버튼·상태 표현을 일관되게 정리하고 좁은 화면에서도 주요 동작을 유지했습니다.
- **쇼핑커넥트 작성 과정 개선**: 상품 내용을 먼저 미리 보고 글감을 저장·관리할 수 있으며, 글별 작성 옵션과 이미지를 한곳에서 확인하는 수동 작성 중심의 흐름으로 단순화했습니다.
- **지능형 추천 저장 안정화**: 추천 기록이 디스크를 계속 차지하지 않도록 저장 크기와 수명을 제한하고, 기존의 비대해진 추천 데이터는 새 저장소가 정상 준비된 뒤 안전하게 정리합니다.

## [0.4.3] - 2026-09-06

### Added
- **카드뉴스 제작**: 블로그·RSS·URL·직접 입력한 내용으로 카드 구성을 만들고, AI 이미지 또는 내 이미지로 완성할 수 있습니다.
- **카드뉴스 관리**: 새 글과 완성·발행 항목을 구분하고, 작업물을 ZIP으로 내보내거나 외부 ZIP을 가져와 이어서 관리할 수 있습니다.
- **SNS 다중 이미지 발행**: 여러 이미지를 미리 보고 순서를 정해 Buffer 채널에 발행할 수 있으며, WordPress 없이도 Google Drive로 공개 이미지를 준비합니다.

### Changed
- **처음 시작하기 개선**: 필요한 설정을 Dashboard에서 바로 확인하고 이동할 수 있으며, 업데이트 후에도 사용자 플랜 정보를 유지합니다.
- **지원 환경 정리**: Electron 44를 적용하고 macOS 13 이상 Apple Silicon과 Windows x64용 설치 파일을 제공합니다.

### Fixed
- **Windows 자동 업데이트 안정화**: 느린 PC에서도 안전하게 업데이트하고 재실행하며, 실패하면 기존 앱을 복구해 원인을 안내합니다.
- **주요 화면 표시 안정화**: 긴 제목, 많은 RSS, 이미지 끌어놓기와 Dashboard 시계 메뉴가 겹치거나 잘리는 문제를 개선했습니다.
...
### Added
- **새로운 블로그 Beta 작업 흐름**: 빠른 글 작성, 네이버 트렌드 탐색, 글감 보관, 발행 대기열과 연속 발행을 하나의 흐름에서 사용할 수 있습니다. 기존 블로그 화면은 그대로 유지해 필요한 방식으로 선택할 수 있습니다.
- **발행 대기열과 연속 발행**: 글마다 발행 채널, 카테고리, 글쓰기 전략, 이미지 처리와 공개 방식을 저장하고 원하는 순서로 관리할 수 있습니다. 허용 시간과 10~360분 사이의 분 단위 간격을 설정하면 준비된 글을 한 건씩 이어서 처리합니다.
- **완성 원고 바로 포스팅**: Markdown 원고 폴더를 선택하거나 완성한 원고를 직접 붙여넣어 미리 본 뒤, AI 재작성 없이 네이버 블로그 또는 WordPress로 발행할 수 있습니다.

### Changed
- **검색 중심 글쓰기와 원고 확인 강화**: 순수 본문 길이를 기준으로 충분한 소제목과 이미지 영역을 구성하고 핵심·관련 키워드를 제목, 도입부, 본문과 마무리에 자연스럽게 분산합니다. Markdown 구조와 이미지 블록도 실제 글에 가깝게 미리 볼 수 있습니다.
- **발행 상태와 사용 준비 상태 개선**: 실행 중인 글감, 다음 처리 시각과 확인이 필요한 상태를 대기열, Blog Beta 메뉴와 시계 영역에서 간결하게 확인할 수 있습니다. Dashboard에서는 네이버·WordPress 연결 상태와 남은 기본 발행 횟수를 한눈에 보여줍니다.

### Fixed
- **실행 중 중복 작업과 작성 내용 손실 방지**: 처리 중인 글감은 수정·이동·제외·중복 실행되지 않도록 보호합니다. 네이버 로그인이 만료되거나 WordPress 연결·권한 확인에 실패해도 작성 중인 글감과 발행 설정을 유지합니다.
...
### Changed
- **대기열 실행 상태 개선**: 자동 또는 수동 처리가 시작되면 해당 글감에 진행 표시와 현재 작업을 보여주고, 완료가 확인될 때까지 목록에 유지합니다. 처리 중에는 글감 수정·순서 변경·대기열 제외·추가 실행과 30초 시험 실행을 막아 실행 대상을 안정적으로 유지하며, `상태 보기`는 빠른 글 작성 결과 옆에서만 간결하게 제공합니다.
- **연속 발행 상태를 어느 화면에서나 확인**: 시계 영역에서 발행 중인 글감, 다음 처리 시각 또는 확인이 필요한 상태를 한 건만 간결하게 확인할 수 있습니다. 발행 중이거나 확인이 필요하면 Blog Beta 메뉴에도 상태 신호가 나타나며, 상태를 누르면 글감 관리로 이동합니다.
- **Dashboard 사용 준비 상태 정리**: 정상 Health와 중복 버전 배지를 제거하고 네이버 로그인, 선택적인 WordPress 설정 여부와 남은 기본 발행 횟수를 하나의 상태 바로 정리했습니다. Google Sheet나 서비스 확인이 필요한 경우에만 관련 안내를 표시하고 설정 또는 로그 화면으로 바로 이동할 수 있습니다.

### Fixed
- **연속 발행 간격 입력 범위 수정**: 발행 간격을 10~360분 사이에서 1분 단위로 설정할 수 있습니다. 11분, 16분과 25분처럼 10분 단위가 아닌 값도 그대로 저장하고 실행합니다.
- **바로 포스팅 실패 시 작성 내용 보존**: 네이버 로그인이 만료되었거나 WordPress 연결·글쓰기 권한을 확인할 수 없으면 발행을 시작하지 않고 작성 중인 글감과 설정을 그대로 유지합니다. 정상적으로 시작된 발행도 완료 전에는 입력 내용을 지우지 않으며, 설정되지 않은 발행 대상은 선택할 수 없습니다.
- **블로그 연결 상태 표시 개선**: 대시보드가 네이버·WordPress의 단순 설정 여부와 실제 연결 확인 결과를 구분하며, 설정을 바꾼 뒤 이전의 정상 표시가 남지 않습니다. 초기 화면은 느린 네이버 브라우저 검사를 기다리지 않고 즉시 `확인 필요` 상태를 안내합니다.

## [0.4.0-dev1] - 2026-09-01

### Added
- **새로운 블로그 Beta 흐름**: 빠른 글 작성, 트렌드 포스팅, 글감 관리와 연속 발행 설정을 하나의 흐름에서 사용할 수 있습니다. 기존 블로그 화면은 그대로 유지해 새 방식을 별도로 시험할 수 있습니다.
- **발행 대기열과 연속 발행**: 작성할 글마다 발행 대상, 카테고리, 글쓰기 전략, 이미지 처리와 공개 방식을 정해 대기열에 추가하고 순서를 바꿀 수 있습니다. 이 기기에서 허용 시간과 발행 간격을 설정하면 준비된 글을 한 번에 한 건씩 이어서 처리합니다.
- **대기열 단건 실행과 진행 확인**: 원하는 글감을 `지금 실행`으로 먼저 처리할 수 있으며 원고 생성, 이미지 준비와 발행 진행 상태를 확인할 수 있습니다. 실행을 시작한 자리에서 `상태 보기`로 진행 영역에 바로 이동할 수 있습니다.
- **완성 원고 바로 포스팅**: Markdown 원고 폴더를 선택하거나 원고를 직접 붙여넣어 검토한 뒤 네이버 블로그 또는 WordPress로 바로 포스팅할 수 있습니다. 완성 원고는 AI로 다시 작성하거나 발행 대기열에 저장하지 않습니다.
- **Beta 트렌드 포스팅**: 네이버 트렌드의 기간과 카테고리를 선택해 글감을 찾고 빠른 작성 또는 글감 보관으로 연결할 수 있습니다. 화면을 오래 열어둔 경우에도 최신 데이터 날짜를 새로 확인할 수 있습니다.

### Changed
- **원고 미리보기 개선**: Markdown의 제목, 소제목, 목록, 인용구, 굵게와 구분선을 실제 글에 가깝게 표시합니다. 이미지 블록은 원고 안의 위치에서 미리 보고, 별도 카드에서 매칭 여부·이미지·설명과 생성 프롬프트를 함께 확인할 수 있습니다.
- **원고 폴더 선택 단순화**: 선택한 폴더와 `원고 폴더 선택` 버튼을 한 줄에 배치하고, 미선택 상태에는 `아직 폴더를 선택하지 않았습니다`를 표시합니다. 선택 해제는 폴더 표시 안의 간결한 동작으로 제공합니다.
- **글쓰기 길이·구성 지침 명확화**: 이미지 생성 프롬프트와 Markdown 표기를 제외한 순수 본문을 목표 글자 수로 사용합니다. 글 길이에 맞는 충분한 소제목과 이미지 영역을 구성하되, 숫자를 맞추기 위한 빈약한 섹션을 만들지 않도록 했습니다.
- **검색 중심 글쓰기 강화**: 핵심 키워드를 제목, 도입 초반, 본문과 마무리에 자연스럽게 분산하고 일부 소제목에도 반영합니다. 관련·보조 키워드는 문맥에 맞는 설명과 실용 정보에 배치해 반복적인 키워드 나열을 줄였습니다.
- **화면 제목 체계 통일**: Dashboard, 블로그, 쇼핑커넥트, SNS, 계정과 설정 화면의 제목과 설명 위치를 블로그 Beta와 같은 구조로 정리했습니다. Dashboard의 재시작·종료 동작은 시계·타이머 메뉴 안에서만 표시됩니다.
- **블로그 Beta 식별 강화**: 사이드바의 Beta 표시에 강조색과 `NEW` 표식을 추가해 기존 블로그와 새 흐름을 쉽게 구분할 수 있습니다.
- **글감 관리 도구 배치 개선**: 새로고침을 탭 오른쪽에 배치하고 보관한 글감과 발행 대기열을 구분해 현재 작업 대상을 더 쉽게 파악할 수 있습니다.

### Fixed
- **트렌드 최신 날짜 갱신**: 앱을 오래 열어둔 동안 새 데이터가 들어와도 날짜 선택 범위가 과거에 머물던 문제를 해결했습니다. 갱신할 때 선택한 카테고리와 유효한 날짜는 유지합니다.
- **발행 상태 동선 개선**: `상태 보기`를 실행 결과 안내 옆에 배치해 화면 끝의 독립 버튼을 찾아야 했던 불편을 줄였습니다.
- **Dashboard 앱 제어 노출 수정**: 앱 시작 직후 기존 재시작·종료 버튼이 제목 영역에 잠시 나타나지 않도록 하고 시계·타이머 메뉴가 열린 경우에만 표시합니다.
