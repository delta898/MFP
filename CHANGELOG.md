...
### Fixed
- **GitHub Actions 릴리스 패키징 안정화**: 플랫폼 빌드에서 전역 `electron-packager`가 성공 코드만 반환하고 산출물을 만들지 않는 경우를 방지하도록 잠금 파일 기반 로컬 도구를 사용하고, 패키징 결과 경로를 다음 단계 전에 명시적으로 검증하도록 보강했습니다.
