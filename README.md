...
**Q. Windows에서 실행 직후 창이 나오지 않거나 안전 모드가 열려요.**
- Windows 배포본은 정상 시작에 실패하면 그래픽 가속과 백그라운드 기능을 끈 안전 모드로 한 번만 다시 시도합니다.
- 안전 모드가 열리면 화면에 표시된 진단 폴더를 확인해 주세요. 기본 위치는 `%LOCALAPPDATA%\BlogGenius`입니다.
- 정상 모드와 안전 모드가 모두 실패하면 진단 ZIP이 만들어지고 해당 위치가 열립니다. 그 ZIP을 개발자에게 전달해 주세요.
- 로그와 크래시 덤프는 자동 전송되지 않으며 시스템·프로세스 정보가 포함될 수 있으므로 전달 전에 내용을 확인해 주세요.
