...
function createContentRouteHandler(deps = {}) {
    ...
    return async function tryHandleContentRoute(ctx = {}) {
        ...
        if (pathname === '/api/v1/settings/shopping-image') {
            ...
            return withError(
                { sendError, res, requestId, status: 400, code: 'SHOPPING_IMAGE_SAVE_FAILED', defaultMessage: '쇼핑 이미지 저장에 실패했습니다.' },
                async () => {
        ...
        if (pathname === '/api/v1/settings/shopping-image/preview') {
            ...
            return withError(
                { sendError, res, requestId, status: 400, code: 'SHOPPING_IMAGE_PREVIEW_FAILED', defaultMessage: '쇼핑 이미지 미리보기에 실패했습니다.' },
                async () => {
        ...
        if (pathname === '/api/v1/settings/google-auth/status') {
            if (!ensureMethod({ method, allowed: 'GET', sendError, res, requestId })) return true;
            return withError(
                { sendError, res, requestId, status: 200, code: 'GOOGLE_AUTH_STATUS_ERROR', defaultMessage: 'Google 인증 상태 조회 중 오류가 발생했습니다.' },
                async () => {
        ...
        if (pathname === '/api/v1/settings/google-auth') {
            if (!ensureMethod({ method, allowed: 'POST', sendError, res, requestId })) return true;
            return withError(
                { sendError, res, requestId, status: 400, code: 'SAVE_FAILED', defaultMessage: '저장 중 오류가 발생했습니다.' },
                async () => {
